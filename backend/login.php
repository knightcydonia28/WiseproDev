                ...
                if (!ctype_alnum($_POST['username'])) {
                    ...
                    if ($stmt->num_rows > 0) {
                        ...
                        if (password_verify($password, $retrieved_password)) {
                            ...
                            include("database.php");
                            $stmt = $DBConnect->prepare("SELECT user_role FROM users WHERE username = ?");
                            $stmt->bind_param("s", $username); 
                            $stmt->execute();
                            $stmt->store_result();
                            $stmt->bind_result($retrieved_user_role);
                            $stmt->fetch();

                            $_SESSION['user_role'] = $retrieved_user_role;

                            $stmt = $DBConnect->prepare("SELECT password_expiration FROM users WHERE username = ?");
                            $stmt->bind_param("s", $username); 
                            $stmt->execute();
                            $stmt->store_result();
                            $stmt->bind_result($retrieved_password_expiration);
                            $stmt->fetch();
