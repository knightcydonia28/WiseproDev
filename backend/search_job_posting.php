...
<html lang="en">
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <input list="clients" name="client_name" id="client_name" placeholder="client" pattern="^[a-zA-Z\s]*$" title="Please ensure that client name has letters and whitespaces only" value="<?php echo $client_name; ?>"><span class="error"> <?php echo $client_name_error; ?></span>
            <datalist id="clients">
            <?php
                include("database.php");
        ...
        <input list="job_titles" name="job_title" id="job_title" placeholder="job title" pattern="^[a-zA-Z\s]*$" title="Please ensure that job title has letters and whitespaces only" value="<?php echo $job_title; ?>"><span class="error"> <?php echo $job_title_error; ?></span>
        ...
        <input list="job_locations" name="job_location" id="job_location" placeholder="job location" pattern="^[a-zA-Z,;\s]*$" title="Please ensure that job location has letters, commas, semicolons, and whitespaces only" value="<?php echo $job_location_pieces[0]; ?>">
        ...
        if (isset($_POST['search_job_posting_submit'])) {
            ...
            if (empty($_POST['vendor_name']) && empty($_POST['client_name']) && empty($_POST['job_title']) && empty($_POST['job_type']) && empty($_POST['job_location']) && empty($_POST['month']) && empty($_POST['year']) && empty($_POST['job_status'])) {
                ...
                if (!empty($_POST['vendor_name'])) {
                    if (!is_numeric($_POST['vendor_name'])) {
                        $vendor_name_error = "Please ensure that the vendor id is numeric";
                    }
                    else {
                ...
                if (!empty($_POST['client_name'])) {
                    if (!is_numeric($_POST['client_name'])) {
                        $client_name_error = "Please ensure that the client id is numeric";
                    }
                    else {
            ...
            include("database.php");
            $stmt = $DBConnect->prepare("SELECT jobs.job_id, vendors.vendor_name, clients.client_name, jobs.job_title, jobs.job_type, jobs.job_location, jobs.job_posted_date, jobs.job_status FROM jobs INNER JOIN clients ON jobs.client_id = clients.client_id LEFT OUTER JOIN vendors ON jobs.vendor_id = vendors.vendor_id WHERE vendors.vendor_name LIKE CONCAT(?, '%') OR clients.client_name LIKE CONCAT(?, '%') OR jobs.job_title LIKE CONCAT(?, '%') OR jobs.job_type LIKE CONCAT(?, '%') OR jobs.job_location LIKE CONCAT(?, '%') OR MONTH(job_posted_date) = ? OR YEAR(job_posted_date) = ? OR jobs.job_status LIKE CONCAT(?, '%')");
            $stmt->bind_param("sssssiis", $vendor_name, $client_name, $job_title, $job_type, $job_location, $month, $year, $job_status);
            $stmt->execute();
            $stmt->store_result();
