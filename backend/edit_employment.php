    ...
    <body onload="checkDropdown()">
                ...
                if (!preg_match("/^[a-zA-Z\s]*$/", $_POST['job_position'])) {
                    ...
                    if ($_POST['employment_type'] != "full-time" && $_POST['employment_type'] != "part-time" && $_POST['employment_type'] != "contract" && $_POST['employment_type'] != "internship") {
                        ...
                        if (!validateDate($employment_start_date)) {
                            $employment_start_date_error = "Please enter a valid employment start date"; 
                        }
                        else {
                            if ($_POST['employment_status'] != "employed" && $_POST['employment_status'] != "unemployed") {
                                ...
                                if ($employment_status == "unemployed" && !empty($_POST['employment_end_date'])) {
                                    ...
                                    if (!validateDate($employment_end_date)) {
                                        $employment_end_date_error = "Please enter a valid employment end date"; 
                                    }
                                    else {
                                        include("database.php");
                                        $stmt = $DBConnect->prepare("UPDATE employments SET job_position = ?, employment_type = ?, employment_start_date = ?, employment_status = ?, employment_end_date = ? WHERE username = ? AND client_id = ?");
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            ...
            <select id="employment_type" name="employment_type" required>
                <option value="">&nbsp;</option>
                <option value="full-time" <?php if (isset($_POST['edit_employment_submit']) && isset($_POST['employment_type']) && $_POST['employment_type'] == "full-time") {echo "selected";} else {if ($retrieved_employment_type == "full-time") {echo "selected";}} ?>>Full-time</option>
        ...
        <?php
            if (!isset($_POST['edit_employment_submit'])) {
                echo
                "<script>
                    document.getElementById(\"employment_start_date\").value = \""; echo $retrieved_employment_start_date; echo "\";
                    document.getElementById(\"employment_end_date\").value = \""; echo $retrieved_employment_end_date; echo "\";
                </script>";
            }
            if (isset($_POST['edit_employment_submit'])) {
                echo
                "<script>
                    ...
                    document.getElementById(\"employment_end_date\").value = \""; echo $_POST['employment_end_date']; echo "\";
                </script>";
            }
        ?>
    </body>
